...
export default function ProjectEditor({ mode, project }: Props) {
  const formContext = useForm({
    ...
    defaultValues: mode === 'edit' && project
      ? {
          ...
          html: project.html,
          skills: project.skills.map(skill => ({ value: skill.name, label: skill.name })),
  ...
  const searchSkills = useCallback(async () => {
    try {
      ...
      setAvailableSkills(uniqBy(response.data.concat(debouncedSkillsSearch ? { name: debouncedSkillsSearch } : []), 'name'))
    }
    catch (error) {
      notifyError(error)
    }
  ...
  const submitProject = useCallback(async (data: z.infer<typeof CreateProjectSchema>, publish = true) => {
    ...
    try {
      ...
      await httpClient.request(mode === 'edit' ? `/projects/${project?.id}` : '/projects', {
  ...
  return (
    <main className="grid lg:grid-cols-[1fr_280px] gap-8">
      <PageToolbar>
        {
          mode === 'create'
            ? (
                <>
                  <Button size="lg" variant="ghost" type="button" disabled={!formContext.formState.isValid || isSubmitting} loading={isSubmitting} onClick={() => formContext.trigger().then(result => void (result && submitProject(formContext.getValues() as CreateProject, false)))}>
                    Save Draft
                  </Button>
            ...
            : (
                <Button size="lg" form="project-form" type="submit" disabled={!formContext.formState.isValid || !formContext.formState.isDirty}>
                  Update Project
                </Button>
              )
        }
      ...
      <FormProvider {...formContext}>
        <form id="project-form" className="project-details w-full flex flex-col gap-4" onSubmit={formContext.handleSubmit(data => submitProject(data))}>
          <FormField
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input className="bg-background" placeholder="https://example.com" prefixEl={<ExternalLinkIcon className="size-3.5" />} {...field} value={field.value || ''} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input className="bg-background" placeholder="https://github.com/..." prefixEl={<GithubIcon className="size-3.5" />} {...field} value={field.value || ''} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
